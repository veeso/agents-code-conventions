...
| Inline table key order     | versioned: version > default-features > features > optional; inherited: workspace > features > optional           |
| `default-features = false` | Set where the version is declared (member crate or `[workspace.dependencies]`); never in a member's inherited dep |
